...
print "\n\nCreate a TCL and AXSf Script for a Defect System:\n\n";
print "\n";
...
while (1) {
   ...
   while (3) {
      $defect_mark = prompt(" Defects to be marked on first frame (O* index(s) will be read, answer \"n\" if there are no defects)? [y/n]");
   ...
   print("\n\nSet Values:");
   print("\n Starting Production Step:    $prod_step");
   print("\n Ending Production Step:      $stop_prod_step");
   print("\n iprint:                      $iprint");
...
my $axsf_out = './config'.$prod_step.'.axsf';

open FNAME, ">$axsf_out";
select FNAME;

#------------------------------------------------------------------------------
# Create the header of the AXSF File
#------------------------------------------------------------------------------
my $num_steps= abs($prod_step - $stop_prod_step)/ $iprint + 1;
print "  ANIMSTEPS          $num_steps\n";


#trimed position array 
my @pos_array = undef;

#Complete position FIle
open INFILE, "<$pos_file" ;
my $read_num = 0;
my $total_read = ($Nat + 1)*$num_steps;
while (<INFILE>) {
   ...
   elsif ( ($read_num > 0) and ($read_num < $total_read) ) {
      push @pos_array, $_;
      #print $_;
      $read_num += 1;
   }
   elsif ( $read_num > $total_read ) {
      last;
   }
}
close INFILE;
...
#------------------------------------------------------------------------------
# Create the main body of the AXSF File
#------------------------------------------------------------------------------
my $step = $prod_step;
my $ncount = 0;
while ($step <= $stop_prod_step) {
   $ncount += 1;
   my $pos=($ncount-1)*$Nat + ($ncount-1) + 2;
   
   @axsf_lines = make_axsf_temp($alat, $Nat, $ncount);

   $na =1;
   foreach $i (0 .. $#axsf_lines) {
      ...
      elsif ($axsf_lines[$i] =~ /NUM/){
         $axsf_lines[$i] =~ s/NUM/$Nat/;
         print "$axsf_lines[$i] \n";
      }
      else
      {
...
if ($defect_num ==  0){
   ...
   foreach $i ( 0 .. $#tcl_lines ) {
      if ($tcl_lines[$i] =~ /OSTAR1/){
         ...
         $tcl_lines[$i] =~ s/OUTPUT/$axsf_out/;
      }
      print "$tcl_lines[$i]\n";
   }


   print STDOUT ("\n\n Creating .axsf file and .tcl script for configuration: $prod_step\n");
...
sub make_tcl_temp {
   ...
   $array[0]  = "scripting::open --axsf OUTPUT";
