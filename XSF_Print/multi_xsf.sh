#! /bin/bash
#--------------------------------------------------------------------
# Script for printing multiple pictures from .xsf files
#  1)Directory of the .XSF Files
#  2)Leading basname of the .xsf Files
#  3)Directory where the files are to go
#  4+)number of states to print ----> These MUST be part of the .xsf file names
#
#--------------------------------------------------------------------

if (( $# == 0 )); then
   echo " "
   echo " Script for printing multiple pictures from .xsf files"
   echo "  1)Directory of the .XSF Files"
   echo "  2)Leading basname of the .xsf Files"
   echo "  3)Directory where the files are to go"
   echo "  4+)number of states to print ----> These MUST be part of the .xsf file names"
   echo " "
   exit
fi

#Get the XSF Directory
XSF_dir=$1
shift
...
#
# File created by charles on localhost.
...
::scripting::open --xsf ${XSF_dir}/${name}_${val}.xsf
...
array set xcMisc {ImageMagick.import /usr/bin/import reduce_to {} rescale_image_list {up down left right center rotXY rotXZ rotYZ rotAB rotAC rotBC wireframes_2d pointlines_2d pipeballs_2d ballsticks2_2d ballsticks1_2d spacefills_2d spacefills_3d ballsticks_3d pipeballs_3d sticks_3d dm_wire dm_solid dm_anaglyph dm_stereo dm_smooth dm_flat rep_unit rep_asym} resolution 1366x768 gif_encoder convert mencoder /usr/bin/mencoder babel /usr/bin/babel status_init_label {Building GUI ...} wm_rootYshift 0 movie_encoder mencoder debug 0 ppmtompeg /usr/bin/ppmtompeg convert /usr/bin/convert titlefile /home/charles/xcrys_tmp/xc_1520/config202100.xsf xwd /usr/bin/xwd resolution_ratio1 1.0 resolutionX 1366 resolution_ratio2 1.0 wm_rootXshift 0 resolutionY 768 ImageMagick.convert /usr/bin/convert}
...
saveState:fontCreate font16 -family {DejaVu Sans} -size -12 -weight normal -slant roman -underline 1 -overstrike 0
...
saveState:fontCreate font20 -family {DejaVu Sans} -size -12 -weight normal -slant roman -underline 1 -overstrike 0
saveState:fontCreate font19 -family {DejaVu Sans} -size -12 -weight normal -slant roman -underline 1 -overstrike 0
saveState:fontCreate TkTooltipFont -family {DejaVu Sans} -size -10 -weight normal -slant roman -underline 0 -overstrike 0
saveState:fontCreate font21 -family {DejaVu Sans} -size 16 -weight normal -slant roman -underline 1 -overstrike 0
...
array set atomLabel {globalFont {} fontBrowser {Simple Font Browser} atomFont.brightID 3 globalFont.darkID 2 globalFont.brightColor {1.0 1.0 1.0} atomIDs.do_no_display.previous {} atomFont.label {} atomFont.do_display 1 atomFont.darkColor {0.0 0.0 0.0} atomFont.entry .atom_label.nb.fatomLabel.atom.e1 atomFont {} globalFont.brightID 1 atomFont.id {} atomTypes.do_not_display {} atomFont.darkID 4 atomFont.brightColor {1.0 1.0 1.0} globalFont.do_display 1 atomIDs.do_not_display {} globalFont.darkColor {0.0 0.0 0.0}}
set t [ModAtomLabels]
...
xc_newvalue .mesa 24 100.000000000000000
...
array set Hbonds {line_patternsize 2 line_pattern 0xaaaa length_min {   1.2000000000 } angle_min { 145.0000000000 } O_like_list {  7   8   9  17 } line_width {   8.0000000000 } length_max {   2.4000000000 } color {0.000 0.000 1.0} H_like_list {  1 }}
HbondsSetting:update .mesa
...
array set check {pseudoDens 0 perspective 1 labels 0 depthcuing 0 crds 0 wigner 0 antialias 0 Hbonds 1 forces 0 frames 0 unibond 1 perpective 0}
CrdSist
AtomLabels
CrysFrames
Unibond
...
array set ws_npos {4 10 8 12 1 8 5 14 10 8 9 8 2 10 6 9 11 8 3 10 7 10}
...
array set DG {envar0,0 1.0 ident,0 3D_PWSCF cb0,0 1 r0 .dg.1.c.f0.r0 YSpace 15 yspace 5 c0,0 .dg.1.c.sf0_0.r l0,0 .dg.1.c.sf0_0.l2 n_block 1 e0,0 .dg.1.c.sf0_0.e bw 410 blockFont font16 bh 44 type,0 3D n_subblock,0 1 subident,0,0 DATAGRID_3D_UNKNOWN ystart 194 radio 0}
DataGridOK
array set isoControl {1,current_slide 1 isoline_color monocolor datagridDim 3 3,cpl_thermoFont font17 1,cpl_thermoNTics 6 3,time_delay 100 2,cpl_thermoNTics 6 3,cpl_thermoNTics 6 3,2Dnisoline 15 cpl_thermoNTics 6 3,anim_step 1 3,cpl_basis MONOCHROME 1,isoline_monocolor #000000 1,isoline_width 2 2,cpl_thermoFont font17 3,cpl_thermoFmt %+8.4f 2,2Dlowvalue 0.000000 2,current_text_slide {Current slide:  1 / 128} 1,cpl_transparency 0 3,colorplane 0 blend_button .iso.fb1.f.right.f5.b3 3,isoline_color monocolor cbfn_apply_to_all 0 1,cpl_thermoFont font17 2,cpl_function LINEAR 1,time_delay 100 current_slide 1 revert_button1 .iso.fb1.f.right.f5.b12 1,2Dnisoline 15 revert_button2 .iso.fb1.f.right.f5.b10a 2Dlowvalue_entry .iso.fb2.f1.1.mf.r.f3.1.entry1 2,cpl_transparency 0 2Dnisoline_entry .iso.fb2.f1.1.mf.r.f5.1.entry1 2,isoline_monocolor #000000 1,2Dhighvalue 2532.899902 cpl_basis MONOCHROME anim_step 1 1,isoline 0 2,isoline 0 3,2Dhighvalue 2532.899902 3,isoline 0 2,cpl_thermoFmt %+8.4f 2,colorplane_lighting 0 isoline_width 2 1,cpl_thermoLabel { Scale:   ? n(r)} 2,cpl_thermoLabel { Scale:   ? n(r)} isoline 0 3,cpl_thermoLabel { Scale:   ? n(r)} colorplane_lighting 0 cpl_thermoTplw 0 1,colorplane 0 cpl_thermoLabel { Scale:   ? n(r)} time_delay 100 3,cpl_transparency 0 2,isoline_color monocolor 2Dnisoline 15 3,current_slide 1 3,cpl_thermoTplw 0 max_allowed_2Dnisoline 100 3,2Dlowvalue 0.000000 cpl_function LINEAR anim_apply_to_all 0 1,anim_step 1 1,cpl_basis MONOCHROME 3,current_text_slide {Current slide:  1 / 128} 3,nslide 128 colorplane 0 cpl_transparency 0 3,isoline_monocolor #000000 3,isoline_width 2 1,cpl_function LINEAR 1,cpl_thermoFmt %+8.4f 2Disolinewidth_entry .iso.fb2.f1.1.mf.i.3.f1.1.entry1 2,cpl_thermoTplw 0 2,time_delay 100 smooth_button .iso.fb1.f.right.f5.b2a 2,2Dnisoline 15 2,nslide 128 1,isoline_color monocolor disp_apply_to_all 0 plane {} 1,isoline_stipple {no stipple} 3Dinterpl_degree 1 2,isoline_stipple {no stipple} 2,current_slide 1 1,cpl_thermoTplw 0 3,isoline_stipple {no stipple} 1,2Dlowvalue 0.000000 isoline_stipple {no stipple} 2Dhighvalue_entry .iso.fb2.f1.1.mf.r.f4.1.entry1 1,current_text_slide {Current slide:  1 / 128} 1,nslide 128 2,colorplane 0 2,anim_step 1 2,cpl_basis MONOCHROME 1,cpl_thermometer 0 2,isoline_width 2 2,cpl_thermometer 0 3,cpl_thermometer 0 2,2Dhighvalue 2532.899902 1,colorplane_lighting 0 bmc .iso.fb2.f1.1.mf.i.1.b color_button .iso.fb1.f.right.f5.b2 cpl_thermometer 0 isoline_monocolor #000000 isosurf 1 3,colorplane_lighting 0 3,cpl_function LINEAR 2Dhighvalue 2532.899902 cpl_thermoFmt %+8.4f cpl_thermoFont font17 2Dlowvalue 0.000000 current_text_slide {}}
array set prop {datagridDim 0 type_of_run RHF pm_isolevel 0 isolevel 10}
array set isosurf {type_of_isosurf solid 3Dinterpl_degree 1 isovalue_entry .iso.fb1.f.left.f1.f3.1.entry1 1,2Dexpand_X 1 transparency on rangevalue 2532.899902 1,2Dexpand_Y 1 1,2Dexpand none res_type angstroms 3,2Dexpand_X 1 1,2Dexpand_Z 1 twoside_lighting off 3,2Dexpand_Y 1 3,2Dexpand none 3,2Dexpand_Z 1 3Dinterpl_degree_old 1 space_sel whole_cell expand_X 1 tessellation_type cubes expand_Y 1 expand whole expand_Z 1 minvalue 0.000000 mb_angs/bohr Angstroms old_twoside_lighting off spin {} 2,2Dexpand_X 1 maxvalue 2532.899902 2,2Dexpand_Y 1 2,2Dexpand none Y_Sel centered 2,2Dexpand_Z 1 2Dexpand_X 1 normals_type gradient shade_model smooth Z_Sel centered 2Dexpand_Y 1 2Dexpand none 2Dexpand_Z 1}
UpdateIsosurf
...
xc_setGLparam material -what isosurf pos front \
   ...
   -diffuse   {1.000000 0.300000 0.300000 0.600000}  \
...
xc_setGLparam material -what isosurf neg front \
   ...
   -diffuse   {0.300000 0.300000 1.000000 0.600000}  \
   -emission  {0.000000 0.000000 0.000000 1.000000}
...
xc_setGLparam material -what isosurf one front \
   -shininess 30.000000 \
   -specular  {1.000000 1.000000 1.000000 1.000000}  \
   -ambient   {0.500000 0.600000 0.500000 1.000000}  \
   -diffuse   {0.700000 0.800000 0.700000 0.600000}  \
   -emission  {0.000000 0.000000 0.000000 1.000000}
xc_setGLparam material -what isosurf one back \
   -shininess 128.000000 \
   -specular  {1.000000 1.000000 1.000000 1.000000}  \
   -ambient   {0.700000 0.700000 0.700000 1.000000}  \
   -diffuse   {0.700000 0.700000 0.700000 0.500000}  \
   -emission  {0.000000 0.000000 0.000000 1.000000}
...
xc_rotationmatrix set  3.003344333235162e-01  3.908444976726625e-01 -8.700803450252396e-01  0.000000000000000e+00 -9.325375797891713e-01  3.119993665166838e-01 -1.817417331658887e-01  0.000000000000000e+00  2.004317600611118e-01  8.659659196135778e-01  4.581813348736636e-01  0.000000000000000e+00  0.000000000000000e+00  0.000000000000000e+00  0.000000000000000e+00  1.000000000000000e+00 
xc_translparam    set  4.953000000000001e+02 -1.284000000000000e+02  1.612848711302319e+01 
...
# Print to File
# ------------------------------------------------------------------------
scripting::printToFile ${pic_dir}/${name}_${val}.png windowdump
